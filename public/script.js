...
function lazyLoadCharts() {
    const capacityTrendChart = document.getElementById('capacityTrendChart');
    const fixtureTypeChart = document.getElementById('fixtureTypeChart');

    const observer = new IntersectionObserver((entries, observer) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                if (entry.target === capacityTrendChart) {
                    renderCharts();
                } else if (entry.target === fixtureTypeChart) {
                    renderCharts();
                }
    ...
    observer.observe(capacityTrendChart);
    observer.observe(fixtureTypeChart);
}